  ...
  var factory = (require) => {
    ...
    function findCommitRow() {
      ...
      for (var i = 0; i < inputs.length; i++) {
        ...
        var input = inputs[i];
        // Step 1: walk up to the first <div> that DIRECTLY contains
        // the input (or an input wrapper). That is the immediate
        // commit input wrapper, which the sidebar uses to stretch
        // the field to fill the row.
        var node = input.parentNode;
        while (node && node !== document.body) {
          if (node.nodeType === 1 && node.tagName === "DIV") {
            // Did this div contain our input directly (or via a small
            // span wrapper)? If yes, keep walking up — we want the
            // ROW, not the input wrapper. The row is the first
            // ancestor div that ALSO has a sibling <button> in the
            // same flex container.
            var hasInput = node.querySelector("input") === input;
            if (!hasInput) {
              node = node.parentNode;
              continue;
            }
            // The current node contains our input. Now check if the
            // current node's PARENT has a <button> sibling. The
            // gitCommit row has structure: <div class=gitCommit>
            //   <div class=gitCommitInput><input/></div>
            //   <button class=gitCommitButton>Commit</button>
            // </div>. So our input's closest-ancestor-with-input is
            // the inner wrapper; the ROW is its parent.
            var row = node.parentNode;
            if (row && row.nodeType === 1) {
              var btn = row.querySelector("button");
              if (btn) return row;
            }
            // Fallback: the input wrapper itself might be the row
            // (older sidebar versions inline everything). Accept it
            // if it has a <button> as a direct child or descendant.
            var btnInline = node.querySelector("button");
            if (btnInline) return node;
            break;
          }
          node = node.parentNode;
        }
