...
try { console.log('[BSPB]', 'client.js loaded at', new Date().toISOString()); } catch(_) {}
//
  ...
  var factory = (require) => {
    ...
    function findCommitRow() {
      ...
      for (var i = 0; i < inputs.length; i++) {
        var ph = placeholderOf(inputs[i]);
        if (isCommitInput(inputs[i])) matched.push(inputs[i]);
        else if (ph) console.log('[BSPB]', 'non-commit input placeholder:', ph);
      }
      console.log('[BSPB]', 'findCommitRow: total inputs', inputs.length, 'matched', matched.length, 'commit inputs');
      if (matched.length === 0) {
        // Log first 3 input placeholders for debugging
        for (var k = 0; k < Math.min(inputs.length, 3); k++) {
          console.log('[BSPB]', '  input[' + k + '] placeholder:', placeholderOf(inputs[k]));
        }
        return null;
      }
      for (var i = 0; i < inputs.length; i++) {
        ...
        while (node && node !== document.body) {
          if (node.nodeType === 1 && node.tagName === "DIV") {
            ...
            if (containsInput) {
              ...
              if (hasDirectButton) {
                console.log('[BSPB]', 'findCommitRow found row at', node.className || node.tagName, 'tag=', node.tagName, 'children=', node.children.length);
                return node;
              } else {
                // The input is inside this div but no direct button. Log why.
                console.log('[BSPB]', '  input in', (node.className || node.tagName), 'but no DIRECT button child; children:', Array.prototype.map.call(node.children, function (c) { return c.tagName; }).join(','));
              }
            }
          }
    ...
    function mountInto(commitRow, ctx) {
      ...
      console.log('[BSPB]', 'mountInto: inserting row after commitRow', commitRow.className || commitRow.tagName, 'parent?', !!commitRow.parentNode);
      var ui = buildRow();
      ...
      console.log('[BSPB]', 'mountInto: after insert, connected=', ui.row.isConnected, 'parent=', commitRow.parentNode && commitRow.parentNode.className, 'nextIsHistory=', ui.row.nextSibling && !!ui.row.nextSibling.querySelector);
      return ui.row;
    }
    ...
    function autoMountBootstrap() {
      ...
      console.log('[BSPB]', 'autoMountBootstrap called');
