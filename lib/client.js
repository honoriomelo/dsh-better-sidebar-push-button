...
(function () {
  var factory = (require) => {
    ...
    // of relying on the class.
    //
    // Walk strategy: start at the input's parent and step up. We stop
    // only when we find a <div> that has BOTH the input's wrapper (or
    // the bare input) as a DIRECT child AND a <button> as a DIRECT
    // child — the fingerprint of the gitCommit flex row (input wrapper
    // + commit button). The two direct-child conditions together keep
    // us from confusing the commit row with anything higher in the
    // tree: the panel root div.git has buttons only nested deeper, and
    // containers like the sidebar root (which does have direct
    // <button>s, e.g. "New Session") do NOT have the commit input's
    // wrapper as a direct child — so even when React is mid-render and
    // has temporarily unmounted the Commit button, we never land on the
    // sidebar header and never put the Push button at the top of the
    // bar.
    // Maximum number of ancestor levels to walk when searching for the
    // commit row. The commit row is always a couple of levels above the
    // input's wrapper (gitCommit > git > aside). A hard cap guarantees we
    // never keep climbing into unrelated page chrome (a workbench toolbar,
    // the sidebar root's own buttons, etc.) when the commit row is
    // mid-render and temporarily lacks its Commit button.
    var COMMIT_ROW_MAX_DEPTH = 8;

    function findCommitRow() {
      var inputs = document.querySelectorAll("input");
      var matched = [];
      for (var i = 0; i < inputs.length; i++) {
        if (isCommitInput(inputs[i])) matched.push(inputs[i]);
      }
      if (matched.length === 0) {
        return null;
      }
      for (var i = 0; i < inputs.length; i++) {
        ...
        var input = inputs[i];
        // The commit row's wrapper: the input's immediate parent. In
        // dsh-better-sidebar the Input component renders a <span>
        // around the <input>, and that span is a DIRECT child of the
        // commit row:
        //
        //   <div class="gitCommit">
        //     <span class="…">            (Input wrapper)
        //       <input class="gitCommitInput" …/>
        //     </span>
        //     <button class="gitCommitButton">Commit</button>
        //   </div>
        //
        // Walk up from the wrapper, one ancestor at a time, and return
        // the first <div> that BOTH has a <button> as a DIRECT child
        // AND has the input's wrapper (or the bare input, when there is
        // no wrapper) as a DIRECT child. Those two conditions together
        // are the fingerprint of the commit row: every higher container
        // either lacks a direct button (div.git) or fails the direct-
        // wrapper test (the sidebar root's New Session button is a
        // direct <button> child, but the commit input's wrapper is
        // buried several levels deeper). The depth cap is a second line
        // of defence for exotic layouts.
        var wrapper = input.parentNode;
        var node = wrapper;
        var depth = 0;
        while (node && node !== document.body && depth < COMMIT_ROW_MAX_DEPTH) {
          if (node.nodeType === 1 && node.tagName === "DIV") {
            // Cheap pre-filter: the div must contain our input at all.
            if (node.querySelector("input") === input) {
              var hasDirectButton = false;
              var hasDirectWrapper = false;
              for (var c = 0; c < node.children.length; c++) {
                var child = node.children[c];
                if (child.tagName === "BUTTON") hasDirectButton = true;
                if (child === wrapper || child === input) hasDirectWrapper = true;
              }
              if (hasDirectButton && hasDirectWrapper) {
                return node;
              }
            }
          }
    ...
    //
    // Three paths to discover { sessionId, cwd, repoRoot }:
    //
    //   (a) [PREFERRED] Observe the sidebar's own fetch calls. The
    //       dsh-better-sidebar client resolves its git scope on the CLIENT
    //       via ctx.sessions.list.getSnapshot().current — the session of the
    //       CURRENT browser tab — and calls /sidebar/api/git.status (and
    //       git.log) with that { sessionId, cwd, repoRoot } payload. That is
    //       exactly the repo the user is looking at (the same one that
    //       feeds the always-correct history card). We wrap window.fetch
    //       once and capture the latest sidebar-issued scope, which is
    //       authoritative and needs no host guessing.
    //
    //   (b) Cordis ctx with a "sessions" service — the path the runner uses
    //       when it does manage to call apply() on us. Synchronous snapshot
    //       read; no network round trip.
    //
    //   (c) Host HTTP route /active-session — last-resort fallback for the
    //       auto-mount bootstrap when neither (a) nor (b) produced a scope
    //       yet. Asynchronous because the host has to consult ctx.agents;
    //       we cache the result for ~1 s so a burst of /check polls does
    //       not stampede the host. NOTE: the host cannot see which browser
    //       tab is active, so this path may target the wrong repo — it is
    //       only used before the sidebar has issued its first git call.
    var ACTIVE_SESSION_CACHE_MS = 1000;
    var activeSessionCache = { value: null, fetchedAt: 0 };
    var observedSidebarScope = null;
    ...
    function installFetchInterceptor() {
      ...
      window.fetch = function (input, init) {
        try {
          ...
          // Never re-capture our own /sidebar/api probes (they carry our
          // possibly-wrong sessionId and would clobber the good scope).
          var isOurs = !!(init && init.headers && init.headers["x-bspb-probe"] === "1");
          if (!isOurs && /\/sidebar\/api\/git\.(status|log|branch|commit|diff)\b/.test(url)) {
            var bodyText = init && typeof init.body === "string" ? init.body : "";
            if (bodyText) {
              var payload = null;
              try { payload = JSON.parse(bodyText); } catch (_) { /* not JSON */ }
              if (payload && payload.sessionId) {
                observedSidebarScope = {
                  sessionId: String(payload.sessionId),
                  cwd: typeof payload.cwd === "string" && payload.cwd ? payload.cwd : undefined,
                  repoRoot: typeof payload.repoRoot === "string" && payload.repoRoot ? payload.repoRoot : undefined,
                };
                // For diag: echo what the sidebar is actually scoped to.
                console.log("[BSPB-DIAG]", "observed sidebar scope", observedSidebarScope);
              }
            }
          }
    ...
    function fetchActiveRoot(ctx) {
      ...
      // Path (a) — sidebar-scope capture: the dsh-better-sidebar client
      // already resolved the correct repo for the current tab. Use its
      // observed repoRoot directly (cheapest) or the sessionId to probe.
      if (observedSidebarScope && observedSidebarScope.sessionId) {
        if (observedSidebarScope.repoRoot) {
          return Promise.resolve({ root: observedSidebarScope.repoRoot });
        }
        return probeSidebarRoot(observedSidebarScope.sessionId, observedSidebarScope.cwd);
      }
      // Path (b) — synchronous ctx snapshot (runner path, real ctx).
      var syncScope = currentScope(ctx);
      if (syncScope && syncScope.sessionId) {
        return probeSidebarRoot(syncScope.sessionId, syncScope.cwd);
      }
      // Path (c) — host HTTP guess (last resort; may pick wrong repo).
    ...
    function probeSidebarRoot(sessionId, cwd) {
      ...
      return fetch("/sidebar/api/git.status", {
        method: "POST",
        headers: { "content-type": "application/json", "x-bspb-probe": "1" },
    ...
    function applyState(ui, info) {
      ...
      if (!info || info.ok !== true) {
        ...
        ui.ahead.style.display = "none";
        console.log('[BSPB-DIAG]', 'applyState DISABLED: ok=false error=', info && info.error);
        return;
      }
      if (info.detached) {
        ui.btn.disabled = true;
        ...
        ui.ahead.style.display = "none";
        console.log('[BSPB-DIAG]', 'applyState DISABLED: detached HEAD');
        return;
      }
      if (!info.upstream) {
        ui.btn.disabled = true;
        ...
        ui.ahead.style.display = "none";
        console.log('[BSPB-DIAG]', 'applyState DISABLED: no upstream, branch=', info.branch);
        return;
      }
      ...
      if (ahead > 0) {
        ...
        console.log('[BSPB-DIAG]', 'applyState ENABLED: ahead=', ahead, 'upstream=', info.upstream);
      } else {
        ui.btn.disabled = true;
        ...
        ui.ahead.style.display = "none";
        console.log('[BSPB-DIAG]', 'applyState DISABLED: ahead=0, upstream=', info.upstream);
      }
    }
        ...
        .then(function (resolved) {
          ...
          return postJson(PUSH_PATH, {
            ...
            if (value.noop) {
              ...
              console.log('[BSPB-DIAG]', 'PUSH ok. exit=', value.exitCode, 'ahead(post)=', n, 'stdout=', (value.stdout || '').slice(0, 300), 'stderr=', (value.stderr || '').slice(0, 300));
              showToast("Pushed " + n + " commit" + (n === 1 ? "" : "s") + " to " + (value.upstream || "upstream") + ".", "success");
            }
            // Apply the post-push state the HOST reported (ahead is now
            // re-inspected AFTER the push, so it reflects the real
            // origin/main vs HEAD difference). If the host says ahead=0
            // the badge clears; if the tracking ref genuinely did not
            // advance (e.g. push was a no-op), we show the truth instead
            // of lying to the user.
            console.log('[BSPB-DIAG]', 'optimistic clear, value.ahead=', value.ahead, 'value.upstream=', value.upstream);
            applyState(ui, {
              ok: true,
              root: resolved.root,
              branch: value.branch || "",
            ...
            console.log('[BSPB-DIAG]', 'after optimistic clear, ahead text=', ui.ahead.textContent, 'display=', ui.ahead.style.display, 'btn disabled=', ui.btn.disabled);
            // Always re-check immediately after a push so the UI flips
            // back to disabled-with-0 as fast as the host can confirm.
            // forceFresh bypasses the in-flight-promise cache so the
            // post-push check cannot be answered by a pre-push snapshot.
            return checkAndApply(ui, ctx, true, true);
          });
        })
        .catch(function (e) {
          done();
    ...
    function checkOnce(ctx, forceFresh) {
      if (forceFresh) IN_FLIGHT_CHECK = null;
      ...
      IN_FLIGHT_CHECK = fetchActiveRoot(ctx)
        .then(function (resolved) {
          ...
          return postJson(CHECK_PATH, {
            ...
            console.log('[BSPB-DIAG]', 'CHECK response ahead=', body.value && body.value.ahead, 'branch=', body.value && body.value.branch, 'upstream=', body.value && body.value.upstream, 'canPush=', body.value && body.value.canPush, 'ROOT=', resolved.root);
    ...
    function checkAndApply(ui, ctx, force, forceFresh) {
      ...
      ui._lastCheckAt = now;
      return checkOnce(ctx, forceFresh).then(function (r) {
        var info = toInfo(r);
        console.log('[BSPB-DIAG]', 'checkAndApply info ahead=', info.ahead, 'upstream=', info.upstream, 'ok=', info.ok, 'canPush=', info.canPush);
        applyState(ui, info);
    ...
    // -- Plugin apply ---------------------------------------------------------
    function apply(ctx) {
      // The bundle may be loaded TWICE in the same page: once by the
      // client-modules table (VM<id>) and once by our injected <script>.
      // Both run this IIFE and both call apply(). Keep exactly one live
      // instance per page — otherwise we get duplicate retry loops and
      // duplicate MutationObservers fighting over the DOM. We pick the
      // first apply() to win; the loser only re-injects styles
      // (idempotent) and returns.
      var ownerFlag = window.__dsh_bspb_apply_owner__;
      if (ownerFlag !== undefined) return; // first apply() already owns it
      window.__dsh_bspb_apply_owner__ = "ours";
      try { injectStyles(); } catch (e) { console.error("[dsh-better-sidebar-push-button] injectStyles failed:", e); }
      // Capture the sidebar's own git scopes as early as possible so the
      // first /check poll already targets the tab's real repo.
      try { installFetchInterceptor(); } catch (e) { console.error("[dsh-better-sidebar-push-button] installFetchInterceptor failed:", e); }
      ...
      // -- Observer scoping -------------------------------------------------
      //
      // dsh-better-sidebar renders its panel as a <div> (the sidebar
      // shell is a <div>, not an <aside>), so findSidebarRoot() can't
      // locate a stable <aside> and we start by observing document.body.
      // As soon as the commit row mounts we re-point the observer at
      // div.git — the commit row's parent — which is the exact subtree
      // that React re-renders on git status/log updates. If React ever
      // replaces div.git itself, the retry loop re-mounts us and this
      // helper re-scopes to the fresh root.
      var bodyObs = null;
      var scopeObserverToGit = function () {
        if (!bodyObs) return;
        var row = findCommitRow();
        if (!row) return;
        var target = row.parentNode;
        if (!target || target === document.body || target === document.documentElement) return;
        if (bodyObs._bspbScopedTo === target) return;
        try {
          var fresh = new MutationObserver(function () { tryMount(); });
          fresh.observe(target, { childList: true, subtree: true });
          fresh._bspbScopedTo = target;
          bodyObs.disconnect();
          bodyObs = fresh;
          console.log('[BSPB-DIAG] observer narrowed to git panel root', target);
        } catch (_) { /* keep the current observer on failure */ }
      };
      ...
      var tryMount = function () {
        try {
          ...
          if (mounted && (!uiHandle || mounted !== uiHandle.row)) {
          ...
          // Narrow the observer to the git panel root now that we know
          // where it lives. Keeping the observer scoped to document.body
          // makes tryMount() run on every render of unrelated React
          // subtrees (chat, settings, …), which — while no longer a
          // mutation loop after the `mounted !== uiHandle.row` fix —
          // still burns cycles and can abort a React commit if we mount
          // mid-render. The git panel root is the smallest container
          // whose childList mutations cover every sidebar re-render we
          // care about.
          scopeObserverToGit();
          return true;
        } catch (e) {
      ...
      // ---- Observer setup -------------------------------------------------
      //
      // Start by observing the sidebar root (aside) when available, or
      // document.body as a fallback. After the first mount,
      // tryMount()->scopeObserverToGit() re-points the observer at the
      // git panel root (div.git) for efficient, tight scoping.
      tryMount();
      ...
      if (root) {
        ...
        bodyObs = new MutationObserver(function () {
          ...
          bodyObs._bspbScopedTo = found;
          tryMount();
        });
        bodyObs.observe(document.body, { childList: true, subtree: true });
        bodyObs._bspbScopedTo = document.body;
      }

      // If the first sync mount already placed the row, narrow now.
      if (uiHandle && uiHandle.row) {
        scopeObserverToGit();
      }

      // ---- Retry loop (safety net) -----------------------------------------
      //
      // The MutationObserver above only starts watching for the sidebar
      // AFTER the first tryMount(). On a hard refresh the whole shell
      // boots asynchronously: our client.js can run before React has
      // mounted anything, so `total inputs 0` — and the observer setup
      // below happens against an empty document.body. Even though
      // document.body is observed, some shells replace the root node or
      // mount into a container that never mutates under body. As a
      // belt-and-braces net, poll for the commit row for up to 60 s and
      // mount the instant it appears. This is cheap (a querySelector
      // per tick) and stops as soon as the row exists.
      var retryAttempts = 0;
      var retryTimer = setInterval(function () {
        retryAttempts++;
        if (retryAttempts > 120) { // 60 s at 500 ms
          clearInterval(retryTimer);
          console.warn('[BSPB] give up: commit row not found after 60 s');
          return;
        }
        if (uiHandle && uiHandle.row) {
          // Already mounted; keep the interval as a re-mount guard but
          // do not spam. We still need it: React may tear down and
          // re-create the commit row, dropping our row from the DOM.
          if (!document.body.contains(uiHandle.row)) {
            uiHandle = null;
            tryMount();
          }
          return;
        }
        var row2 = findCommitRow();
        if (row2) {
          clearInterval(retryTimer);
          tryMount();
        }
      ...
      // Expose the stop handle so pagehide / takeover can clear it.
      var flag = window[AUTO_MOUNT_FLAG] || (window[AUTO_MOUNT_FLAG] = {});
      flag.stopRetry = function () { clearInterval(retryTimer); };
