...
// Trigger the bootstrap immediately when the script loads. The factory
// above is registered with __ModuleLoader__; it does not run until the
// client-modules subsystem materializes the module table, so we have to
// drive the bootstrap from outside the factory.
//
// We re-implement `apply()` here against the same DOM logic via a tiny
// direct script entry point that the __DSH_BOOT__ HTML loads separately
// from the cordis-managed script. To avoid that complication, the cleanest
// workaround is: when the factory eventually runs (because the module
// table materializes this id), `autoMountBootstrap` is what kicks `apply()`.
// That function is only reachable from inside the factory, so we use a
// deferred driver that:
//   1. installs a global "auto mount requested" flag NOW,
//   2. waits for `__ModuleLoader__.create()` to bootstrap the module
//      system (it consults our pending queue and materializes us),
//   3. invokes the auto-mount via a side door the factory exposes.
//
// The side door is the `window.__dsh_bspb_requestAutoMount__` function:
// the factory installs it on the first call, and our driver invokes it
// here. This indirection keeps all module-table state inside the factory
// while still letting the script drive the bootstrap at top level.
if (typeof window !== "undefined") {
  // Guard: only the first script load (per page) installs the driver.
  if (!window.__dsh_bspb_driverInstalled__) {
    window.__dsh_bspb_driverInstalled__ = true;
    var attempt = 0;
    function tryAutoMount() {
      attempt++;
      var installer = window.__dsh_bspb_requestAutoMount__;
      if (typeof installer === "function") {
        try { installer(); }
        catch (e) { console.error("[dsh-better-sidebar-push-button] auto-mount invoke failed:", e); }
        return;
      }
      // The factory has not run yet. The client-modules create() call
      // drains our pendingQueue and materializes the factory, which
      // installs __dsh_bspb_requestAutoMount__. Wait up to ~30 s for it.
      if (attempt > 600) {
        console.warn("[dsh-better-sidebar-push-button] auto-mount did not install within 30 s; the cordis-client-runner may have refused the bundle (check the runner logs).");
        return;
      }
      setTimeout(tryAutoMount, 50);
    }
    // Defer to give the runner a chance to call apply() first if it is
    // going to (in which case the factory's singleton guard will skip
    // our auto-mount).
