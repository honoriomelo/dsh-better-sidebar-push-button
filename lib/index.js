    ...
    return ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          // 'https://github.com'". Retry once by injecting the gh CLI token
          // into the remote URL (same as a manual terminal push with
          // `git push https://<user>:$(gh auth token)@github.com/…`).
          let ghToken = null;
          if (isCredentialError(pushResult)) {
            try {
              const authed = await authedPushUrl(ctx, resolved.root, info.upstream);
              if (authed && authed.authed) {
                ghToken = authed.token || null;
                // Pass the branch explicitly so the retry never depends on
                // the user's push.default config: `git push <url> <branch>`.
                const retried = await runGit(ctx, resolved.root, [
                  '--no-pager', 'push', '--progress', authed.authed, info.branch,
                ], PUSH_TIMEOUT_MS, undefined, 'danger-full-access');
                if (retried && typeof retried.exitCode === 'number' && retried.exitCode === 0) {
                  pushResult = retried;
                  // A push to an explicit URL does NOT update the local
                  // remote-tracking ref (refs/remotes/origin/<branch>), so
                  // /check would keep reporting ahead>0 even though the
                  // commits are on GitHub. Sync the tracking ref with a
                  // fetch using the same authenticated URL.
                  try {
                    await runGit(ctx, resolved.root, [
                      '--no-pager', 'fetch', authed.authed,
                      '+refs/heads/' + info.branch + ':' + 'refs/remotes/' + info.upstream,
                    ], PUSH_TIMEOUT_MS, undefined, 'danger-full-access');
                  } catch (_) { /* fetch is best-effort; inspectUpstream handles staleness */ }
                } else {
                  pushResult = retried || pushResult;
                }
              }
            } catch (e) {
              // Keep the original error; the token retry is best-effort.
            }
          }
