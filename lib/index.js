...
function describeFailure(result, fallback) {
  ...
  if (combined) {
    ...
    if (/Bad owner or permissions on .*ssh_config/.test(combined)
      ...
      return 'SSH config permissions are blocking git. Fix with `sudo chmod 644 /etc/ssh/ssh_config.d/*` (or remove the offending symlink) and retry, or switch the remote to HTTPS: `git remote set-url origin https://github.com/<user>/<repo>.git`. Raw error: ' + combined;
    }
...
function apply(ctx, config) {
  ...
  ctx.on('webserver/index-inject', (table) => {
    table.push({
      kind: 'script-src',
      placement: 'head',
      src: '/plugins/dsh-better-sidebar-push-button/client.js',
    });
  });
