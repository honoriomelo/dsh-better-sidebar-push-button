...
function apply(ctx, config) {
  ...
  // registered there, and even though `__DSH_BOOT__` lists our id, no
  // `<script src=...>` tag is emitted by the shell for non-`@deepseek-ai`
  // entries. Our factory therefore never executes, and `apply()` never
  // runs.
  //
  // We close the gap by tapping the index.html response and injecting a
  // `<script>` that loads our client.js alongside the other plugins. The
  // client.js top-level then runs the factory itself (see lib/client.js
  // for the dual-consumer pattern). When the runtime eventually grows a
  // proper static-bundle auto-mount, this tap becomes a no-op for our id
  // because the runner takes over before our second invocation.
  ctx.effect(() => {
    return ctx.webServer.tapIndex(function (html) {
      // Skip if some other tap already loaded our client (idempotent).
      if (html.indexOf('/plugins/dsh-better-sidebar-push-button/client.js') !== -1) return html;
      // Inject before the bundle's own main module so the bootstrap
      // runs as soon as the HTML head reaches a stable state. The shell
      // exposes `__DSH_BOOT__` and the cordis module system inline;
      // appending a plain `<script src=...>` keeps execution order
      // deterministic.
      const tag = '<script src="/plugins/dsh-better-sidebar-push-button/client.js"></script>';
      // Prefer injecting right before the shell's own main module script
      // (the one with `type="module"`). Fall back to `</head>`.
  ...
  }, 'dsh-better-sidebar-push-button: index tap');
